      ...
      <div className="flex gap-2 p-4 pt-0 sm:flex-col">
        {items?.map((item) => {
          const pickupDate = new Date(
            item?.pickupTime?.toString().replace("Z", "") ?? new Date(),
          );
          ...
          return (
            ...
            >
              ...
              <div className="inline-flex gap-1 text-xs font-bold">
                Total
                <span className="space-x-1 font-black">
                  <span>{item.totalAmount ?? "0.00"}</span>
                  <span className="font-thin">PEN</span>
                </span>
              </div>
              <div className="inline-flex items-center justify-center gap-2">
                ...
                />
                <span>
                  {`${format(pickupDate, "EEEE, dd MMMM", { locale: lang === "es" ? es : undefined })}, ${hour12}:${minute} ${ampm}`}
                </span>
              </div>
            </button>
          );
